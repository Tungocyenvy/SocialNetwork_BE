...
cron.schedule('*/1 * * * * *', async() => {
  let now = new Date();
  let expirationDate =new Date(now.setDate(now.getDate() - 90));
  const account = await Account.find({isDelete:true, deletedDate:{$gte:expirationDate}})
    // console.log("🚀 ~ file: schedule.service.js ~ line 33 ~ cron.schedule ~ account", account)
  });