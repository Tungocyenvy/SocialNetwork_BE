...
const getFacultyByUserId = async (req, res, next) => {
  const UserID = req.value.body.token.data;
  const lang = req.headers['accept-language'];
  const resService = await groupService.getFacultyByUserId(UserID,req, lang);
  if (resService.statusCode === 200) {
    return controller.sendSuccess(
      res,
      resService.data,
...
//change alummi
const changetoAlumni = async (req, res, next) => {
  const lang = req.headers['accept-language'];
  const resService = await groupService.changetoAlumni(req.body, lang);
  if (resService.statusCode === 200) {
    return controller.sendSuccess(
      res,
      resService.data,
